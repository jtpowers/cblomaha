...
for (var i=0;i<buttons.length;i++) {
 width+=buttons[i]
 s+="<SPAN STYLE='position:relative;height:26;width: " + buttons[i] + "'><SPAN STYLE='position:absolute;margin:0px;padding:0;height:26;top:0;left:0;width:" + (buttons[i]) + ";clip:rect(0 "+buttons[i]+" 25 "+0+");overflow:hidden'><IMG BORDER=0 SRC='../wsimages/htmleditortoolbar.gif' STYLE='position:absolute;top:0;left:-" + left + "' WIDTH=317 HEIGHT=99"
 if (buttons[i]!=4) {
   switch (action[i]) {
     case "createLink":
